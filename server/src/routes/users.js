...
const { verifyToken, verifyTokenNext, verifyTokenBoolean } = require('../global_functions/auth');
...
app.put('/:userId/subscription', (req, res) => {
  ...
  User.findById(userId)
    .then((user) => {
      ...
      user.save().then(user => { res.json(user); }).catch(er => { console.log(er); res.status(500).json({}) })
...
app.post('/wishlist', verifyToken, async (req, res) => {
  ...
  User.findById(userId)
    .then((user) => {
      ...
      if (index !== -1) {
        // Remove the ID from user's wishlist
      ...
      user.save()
        ...
        .catch((error) => {
          console.log(error);
          res.status(500).json({ error: 'An error occurred while saving the wishlist' });
        });
    })
    .catch((error) => {
      console.log(error);
...
//wishlist Get
app.get('/wishlist/:id', verifyToken, (req, res) => {
  const id = req.params.id;
  const userId = req.userId;
    ...
    .then((user) => {
      ...
      if (index === -1) {
        ...
        user.save()
          ...
          .catch((error) => {
            console.log(error);
            res.status(500).json({ error: 'An error occurred while saving the Recently Watched' });
          });
      } else {
